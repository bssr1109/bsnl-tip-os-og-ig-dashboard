...
def bbm_dashboard():
    ...
    if not month_list:
        ...
        df_m = sheets[month_sel].copy()
        if df_m.empty:
            st.info("No records in selected month.")
        else:
            # Backwards‑compatibility for old status files
            if "BBM_STD" not in df_m.columns:
                if "BBM" in df_m.columns:
                    df_m["BBM_STD"] = df_m["BBM"].astype(str).str.upper().str.strip()
                else:
                    st.info("Status file has no BBM information (BBM_STD / BBM).")
                    df_m = pd.DataFrame()

            if not df_m.empty:
                # Filter only this BBM
...
def mgmt_dashboard():
    ...
    # Backwards‑compatibility for old status files
    if "TIP_STD" not in df_m.columns and "TIP_NAME_STD" in df_m.columns:
        df_m["TIP_STD"] = df_m["TIP_NAME_STD"]
    if "BBM_STD" not in df_m.columns:
        if "BBM" in df_m.columns:
            df_m["BBM_STD"] = df_m["BBM"].astype(str).str.upper().str.strip()
        else:
            # If no BBM column at all, create a placeholder so grouping works
            df_m["BBM_STD"] = "UNKNOWN"
