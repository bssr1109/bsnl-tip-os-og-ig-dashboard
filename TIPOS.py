...
try:
    ...
    if os.path.exists("tip_users.json"):
        with open("tip_users.json", "r", encoding="utf-8") as f:
            TIP_USERS = json.load(f)  # { "TIP NAME": "TIP4155", ... }
...
def login_form():
    ...
    if not MGMT_PASSWORD and not BBM_USERS and not TIP_USERS:
        st.error("Login JSON files (mgmt.json, bbm_users.json, tip_users.json) not loaded. Please keep them in the same folder as TIPOS.py.")
        return
    ...
    with st.form("login_form"):
        ...
        if role == "MGMT":
            if not MGMT_PASSWORD:
                st.error("❌ Management password not configured in mgmt.json")
                return
...
with col_logout:
    if st.button("🚪 Logout"):
        ...
        for key in [
            "authenticated", "role", "username", "current_bbm",
            "os_df", "og_df",
            "os_filename", "og_filename",
            "os_uploaded_at", "og_uploaded_at",
            "os_uploaded_by", "og_uploaded_by",
            "status_sheets",
        ]:
...
def preprocess(os_df, og_df):
    ...
    # TIP/BBM views are restricted to current BBM if set
...
def tip_view():
    ...
    if not os_tip_raw.empty:
        bbm_values.extend(os_tip_raw[COL_OS_BBM].astype(str).str.upper().str.strip().tolist())
    if not og_tip_raw.empty:
        bbm_values.extend(og_tip_raw[COL_OG_BBM].astype(str).str.upper().str.strip().tolist())
    bbm_values = sorted(set(bbm_values))

    current_bbm_filter = st.session_state.get("current_bbm", "")
    bbm_options = ["ALL BBMs"] + bbm_values
    if current_bbm_filter and current_bbm_filter in bbm_values:
        default_index = bbm_options.index(current_bbm_filter)
    else:
        default_index = 0

    st.markdown("### 🧭 Filter by BBM")
    selected_bbm = st.selectbox(
        "Select BBM (applies to OS / OG data and contact logs):",
        options=bbm_options,
        index=default_index,
    )

    new_bbm_filter = "" if selected_bbm == "ALL BBMs" else selected_bbm.upper().strip()
    if new_bbm_filter != current_bbm_filter:
        st.session_state.current_bbm = new_bbm_filter
        st.rerun()

    bbm_name = st.session_state.get("current_bbm", "")

    # Now use preprocessed data (already filtered by current_bbm)
    ...
    if tip_os.empty and tip_og.empty:
        st.error(
            f"No records found for TIP Name: `{tip_name}` "
            f"{'(BBM: ' + bbm_name + ')' if bbm_name else ''}.\n"
            "Check that login name exactly matches the TIP name in Excel and BBM has uploaded his data."
        )
        return

    st.subheader(f"📌 TIP Dashboard – {tip_name} (BBM: {bbm_name or 'ALL'})")
    ...
    if CURRENT_MONTH in sheets:
        ...
        if not df_month.empty and "BBM_STD" in df_month.columns:
            df_tip_log = df_month[
                ...
                ((df_month["BBM_STD"] == bbm_filter) if bbm_filter else True)
            ].copy()

    if df_tip_log.empty:
        st.info("No Call / WhatsApp actions recorded for you in this month (for the selected BBM filter).")
    else:
        ...
        st.caption("🧾 Detailed log (per account) – this TIP, this BBM filter, this month:")
